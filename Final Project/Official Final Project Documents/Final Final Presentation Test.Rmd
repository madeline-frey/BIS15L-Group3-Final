...
# Turtle Dataset
We are using data collected by NOAA Fisheries' Southeast Fisheries Science Center. They characterized North Carolina populations of sea turtles between 1988 and 2015.
...
# How did turtle captures change over time?
...
turtles3%>%
  ...
  theme_light(base_size = 12)+
     theme(axis.text.x = element_text(angle = 60, hjust = 1))
```
Turtles are caught most in October and November. Does this trend hold up across years?
```{r,echo=FALSE}
turtles3%>%
  ggplot(aes(month(x=date_capture_new,label=TRUE),fill=month(x=date_capture_new,label=TRUE)))+
  geom_bar()+
  ...
  facet_wrap(~year)
```
Yes, the trend seems to be consistent in most years, with some exceptions. 
```{r}
...
```
- Loggerhead turtles peaked in 2001, then quickly dropped off. 
- Green sea turtles were caught much more frequently from 2007-2009. 
- Other species did not vary as much.
...
turtles3%>%
  ...
  theme_light(base_size = 12)+
     theme(axis.text.x = element_text(angle = 60, hjust = 1))
```
The most turtles were caught between the 10th and 20th days of the month. 
...
turtles3%>%
  ...
  theme_light(base_size = 12)+
     theme(axis.text.x = element_text(angle = 60, hjust = 1))
```
Monday and Friday were big days for turtles!
